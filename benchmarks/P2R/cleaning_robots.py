...
from sgrk_p2r import format_sgrk_p2r
...
if len(sys.argv) < 2:
    ...
    # P2R implications: FG assumptions -> GF guarantees
    # ((FG in:done & FG !in:clean_i) -> (GF out:clean_i))
    impls_1 = [([Var(in_done),
                 Not(Var(in_clean[i]))],
                [Var(out_clean[i])])
               for i in range(n_rooms)]
    ...
    print(format_sgrk_p2r(in_init, out_init, in_trans, out_trans, impls))
