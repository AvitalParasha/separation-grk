...
from sgrk_r2p import format_sgrk_r2p
...
if len(sys.argv) < 2:
    ...
    # R2P implications: GF assumptions -> FG guarantees
    # ((GF in:done & GF !in:clean_i) -> (FG out:clean_i))
    impls_1 = [([Var(in_done),
                 Not(Var(in_clean[i]))],
                [Var(out_clean[i])])
               for i in range(n_rooms)]
    ...
    print(format_sgrk_r2p(in_init, out_init, in_trans, out_trans, impls))
